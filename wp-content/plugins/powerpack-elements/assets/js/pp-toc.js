(function ($) {
	var PPToCHandler = elementorModules.frontend.handlers.Base.extend({
		...
		setOffset: function setOffset( $listItem ){
			...
			var offset = this.getElementSettings("scroll_offset");
		...
		collapseOnInit: function collapseOnInit() {
			...
			if ("" !== minimizedOn && "array" === typeof minimizedOn) {
				minimizedOn.forEach(function (value) {
					if (
						("tablet" === value && "tablet" === currentDeviceMode) ||
						("mobile" === value && "mobile" === currentDeviceMode) ||
						("desktop" === value && "desktop" == currentDeviceMode)
					) {
						self.collapseBox();
					}
				});
		...
		run: function run() {
			...
			if(elementSettings.scroll_offset) {
				this.setOffset();
			}
		},
