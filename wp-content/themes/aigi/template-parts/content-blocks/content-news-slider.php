...
<section class="rslider-section resources-slider wrapper-full-width acf-section-<?php echo get_row_index() . ' '; ?> acf-section-<?php echo $attributes['uniq_id']. ' '; ?><?php echo $classes ?><?= $background_texture; ?><?= $padding; ?><?= $border; ?> <?= $bg_color_preset; ?>" id="<?php  echo ($attributes['section_id'] ? $attributes['section_id'] :''); ?>"  style="background-image: url(<?php echo $attributes['background']['background_image']['url']; ?>); <?php  echo ($bg_color ?  'background-color: ' . $bg_color . ';' :''); ?> ">
                        ...
                        <div>
                            <div class="rslider__item rslider__resource-item <?php echo $post_type; ?>">
                                ...
                                <div class="rslider__item-body">
                                    <a class="rslider__title" href="<?=  get_the_permalink($post->ID) ?>" data-height="NewsrsliderTitle"><?= $post->post_title; ?></a>
                                    <div class="rslider__excerpt" data-height="NewsrsliderDesc"><?php echo get_custom_excerpt($post->post_excerpt, 94, true) ?></div>
                                    <div class="rslider__tags" data-height="NewsrsliderTags">
                                        <?php $term_list = wp_get_post_terms( $post->ID, 'content_tags', array('fields' => 'all') );
                                        foreach ($term_list as $term) : ?>
                                            <a class="rslider__tags-item" href="/search?_content_tags=<?php echo $term->slug ?>" data-tem-id="<?php echo  $term->term_id ?>"><?php echo $term->name ?></a> <span class="coma">,</span>
                                        <?php endforeach ?>
                                    </div>
                                </div>
